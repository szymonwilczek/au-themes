  ...
  (let* ((pal (au-extract-active-palette))
         ...
         (tokens
          (if (eq polarity 'light)
              '(("Base text (Conifer bark shadow)"  :fg-main      58.0 85.0)
                ("Comments (Misty lichen mulch)"    :fg-dim       35.0 60.0)
                ("Cursor (Mountain rain glint)"     :cursor       35.0 75.0)
                ("Preprocessor (#define)"           :preprocessor 40.0 78.0)
                ("Keywords (struct, while)"         :keyword      40.0 78.0)
                ("Data types (int, size_t)"         :type         40.0 78.0)
                ("Constants (LOTA_PCR_COUNT)"       :constant     40.0 78.0)
                ("Numbers (0, 24, 32)"              :number       40.0 78.0)
                ("Builtins (__always_inline)"       :builtin      40.0 78.0)
                ("Function definitions"             :fnname       40.0 78.0)
                ("Function calls (bpf_...)"         :fnname-call  40.0 78.0)
                ("Strings (\"string literals\")"    :string       40.0 78.0)
                ("Struct fields (->tgid)"           :property     40.0 78.0)
                ("Operators (+, -, *, >>)"          :operator     40.0 78.0)
                ("Brackets (( ) [ ] { })"           :bracket      40.0 78.0)
                ("Alerts / Errors (!)"              :err          40.0 78.0))
    ...
    (when (eq polarity 'light)
      (let* ((bg-y (rf-luminance-y bg))
             (bg-ok (and (>= bg-y 0.4000) (<= bg-y 0.7500))))
