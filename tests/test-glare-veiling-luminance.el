  ...
  (let* ((pal (au-extract-active-palette))
         ...
         (tokens
          (if (eq polarity 'light)
              (if (< bg-y 0.420)
                  '(("Comments (Misty lichen mulch)"    :fg-dim       25.0 60.0)
                    ("Brackets (( ) [ ] { })"           :bracket      30.0 78.0)
                    ("Keywords (struct, while)"         :keyword      30.0 78.0)
                    ("Data types (int, size_t)"         :type         30.0 78.0)
                    ("Preprocessor (#define)"           :preprocessor 30.0 78.0)
                    ("Base text (Conifer bark shadow)"  :fg-main      48.0 85.0))
