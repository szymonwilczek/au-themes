  ...
  (let* ((pal (rainforest-extract-active-palette))
         ...
         (tokens
          (if (eq polarity 'light)
              '(("Base text (Conifer bark shadow)"  :fg-main      0.25 0.38 0.01 0.08)
                ("Comments (Misty lichen mulch)"    :fg-dim       0.45 0.60 0.01 0.08)
                ("Cursor (Mountain rain glint)"     :cursor       0.35 0.60 0.05 0.20)
                ("Preprocessor (#define)"           :preprocessor 0.38 0.55 0.04 0.20)
                ("Keywords (struct, while)"         :keyword      0.36 0.55 0.06 0.22)
                ("Data types (int, size_t)"         :type         0.38 0.55 0.06 0.22)
                ("Constants (LOTA_PCR_COUNT)"       :constant     0.36 0.55 0.06 0.22)
                ("Numbers (0, 24, 32)"              :number       0.38 0.55 0.06 0.22)
                ("Builtins (__always_inline)"       :builtin      0.36 0.55 0.04 0.20)
                ("Function definitions"             :fnname       0.38 0.55 0.04 0.20)
                ("Function calls (bpf_...)"         :fnname-call  0.38 0.55 0.05 0.20)
                ("Strings (\"string literals\")"    :string       0.38 0.55 0.05 0.20)
                ("Struct fields (->tgid)"           :property     0.36 0.55 0.03 0.16)
                ("Operators (+, -, *, >>)"          :operator     0.36 0.55 0.01 0.10)
                ("Brackets (( ) [ ] { })"           :bracket      0.36 0.55 0.01 0.10)
                ("Alerts / Errors (!)"              :err          0.36 0.55 0.08 0.25))
            '(("Base text (Mineral quartz)"       :fg-main      0.60 0.85 0.01 0.08)
              ("Comments (Damp needles)"          :fg-dim       0.30 0.45 0.01 0.08)
              ("Cursor (Raindrop glint)"          :cursor       0.50 0.70 0.06 0.20)
              ("Preprocessor (#define)"           :preprocessor 0.55 0.75 0.08 0.22)
              ("Keywords (struct, while)"         :keyword      0.40 0.58 0.08 0.25)
              ("Data types (int, size_t)"         :type         0.50 0.70 0.09 0.25)
              ("Constants (LOTA_PCR_COUNT)"       :constant     0.40 0.60 0.08 0.25)
              ("Numbers (0, 24, 32)"              :number       0.55 0.75 0.10 0.25)
              ("Builtins (__always_inline)"       :builtin      0.45 0.65 0.06 0.20)
              ("Function definitions"             :fnname       0.40 0.60 0.05 0.18)
              ("Function calls (bpf_...)"         :fnname-call  0.50 0.70 0.07 0.20)
              ("Strings (\"string literals\")"    :string       0.48 0.68 0.08 0.25)
              ("Struct fields (->tgid)"           :property     0.50 0.68 0.04 0.16)
              ("Operators (+, -, *, >>)"          :operator     0.42 0.58 0.01 0.08)
              ("Brackets (( ) [ ] { })"           :bracket      0.35 0.50 0.01 0.08)
              ("Alerts / Errors (!)"              :err          0.40 0.60 0.10 0.28)))))
    (princ (format "\n======================================================================\n"))
