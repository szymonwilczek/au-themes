      ...
      (let* ((partial (cond
        ...
        (list
         ...
         :preprocessor (or (cadr (assq 'yellow-cooler partial))
                           (rainforest-get-theme-face-fg theme 'font-lock-preprocessor-face)
                           "#7a5028")
         :keyword (or (cadr (assq 'green-cooler partial))
                      (cadr (assq 'cyan partial))
                      (rainforest-get-theme-face-fg theme 'font-lock-keyword-face)
                      "#2c8446")
         :type (or (cadr (assq 'green partial))
                   (rainforest-get-theme-face-fg theme 'font-lock-type-face)
                   "#54a648")
         :constant (or (cadr (assq 'green-warmer partial))
                       (rainforest-get-theme-face-fg theme 'font-lock-constant-face)
                       "#6e9a38")
         :number (or (cadr (assq 'yellow-warmer partial))
                     (rainforest-get-theme-face-fg theme 'font-lock-number-face)
                     "#bfa03c")
         :builtin (or (cadr (assq 'blue-faint partial))
                      (rainforest-get-theme-face-fg theme 'font-lock-builtin-face)
                      "#286cb8")
         :fnname (or (cadr (assq 'blue partial))
                     (rainforest-get-theme-face-fg theme 'font-lock-function-name-face)
                     "#1e72cc")
         :fnname-call (or (cadr (assq 'blue-warmer partial))
                          (rainforest-get-theme-face-fg theme 'font-lock-function-call-face)
                          "#3282dc")
         :string (or (cadr (assq 'red-warmer partial))
                     (rainforest-get-theme-face-fg theme 'font-lock-string-face)
                     "#966919")
         :property (or (cadr (assq 'fg-alt partial))
                       (rainforest-get-theme-face-fg theme 'font-lock-property-name-face)
                       "#a48e38")
         :operator (or (cadr (assq 'blue-cooler partial))
                       (rainforest-get-theme-face-fg theme 'font-lock-operator-face)
                       "#587462")
         :bracket (or (cadr (assq 'red-faint partial))
                      (rainforest-get-theme-face-fg theme 'font-lock-bracket-face)
                      "#567060")
         :delimiter (or (cadr (assq 'cyan-faint partial))
                        (rainforest-get-theme-face-fg theme 'font-lock-delimiter-face)
                        "#cfbcba")
         :err (or (cadr (assq 'red partial))
                  (rainforest-get-theme-face-fg theme 'font-lock-warning-face)
                  "#b43a34"))))))
