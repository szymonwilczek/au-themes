...
(defun rainforest-extract-active-palette (&optional theme-name)
  ...
  (let* ((theme (or theme-name rf-active-theme 'rainforest-night))
         ...
         (hl (or (cadr (assq 'bg-hl-line partial)) (if (eq theme 'rainforest-day) "#a2af9f" "#141815"))))
    (list
     :theme theme
     :bg-main bg
     :bg-hl-line hl
